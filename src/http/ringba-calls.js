...
/**
 * Fetch calls with generic filters and columns
 * 
 * @param {string} accountId 
 * @param {string} apiToken 
 * @param {Object[]} filters - Ringba API filter objects
 * @param {string[]} columns - Array of column names to fetch
 * @param {Object} options 
 * @returns {Promise<Either<Error, {calls, summary}>>}
 */
export const getCalls = (accountId, apiToken) => (filters, columns, options = {}) =>
    TE.tryCatch(
        async () => {
            ...
            console.log(`[Ringba] Fetching calls with filters: ${JSON.stringify(filters)}`);
        ...
        (error) => new Error(`Failed to fetch calls: ${error.message}`)
    );
