...
// Script to run Ringba Campaign Summary sync service V2 (using Insights API)
// Fetches and saves campaign summary data (RPC, total calls, totalCost, etc.) for a specific date

import { fileURLToPath } from 'url';
import { dirname, join } from 'path';
import dotenv from 'dotenv';
import { syncCampaignSummary, syncCampaignSummaryByCampaignId } from './src/services/ringba-campaign-summary-v2.js';
  ...
  try {
    ...
    if (isCampaignId) {
      ...
      if (!date) {
        ...
        console.log('');
        console.log('Usage: node run-ringba-campaign-summary-v2.js <campaignId> [date]');
        console.log('');
      ...
      if (!date) {
        console.error(`❌ Error: Invalid date format: ${dateStr}`);
        console.log('');
        console.log('Usage: node run-ringba-campaign-summary-v2.js [date]');
        console.log('   or: node run-ringba-campaign-summary-v2.js <campaignId> [date]');
        console.log('');
        console.log('Date formats supported:');
        console.log('  - YYYY-MM-DD (e.g., 2025-11-20)');
        console.log('  - MM/DD/YYYY (e.g., 11/20/2025)');
